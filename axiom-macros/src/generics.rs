...
use syn::{Type, PathArguments, GenericParam, Generics, Lifetime};
...
/// Parse type bounds
fn parse_type_bounds(bounds: &syn::punctuated::Punctuated<syn::TypeParamBound, syn::token::Plus>) -> Vec<GenericBound> {
    bounds
        .iter()
        .filter_map(|bound| {
...
fn parse_lifetime_bounds(bounds: &syn::punctuated::Punctuated<Lifetime, syn::token::Plus>) -> Vec<GenericBound> {
    bounds
        .iter()
...
/// Extract inner type from generic wrapper (e.g., Option<T> -> T)
...
/// Generate where clause for generic constraints
