...
fn main() {
    ...
    println!("  ⏱️  get_many({} keys) x100:  {:?}", GET_MANY_SIZE, elapsed);
    ...
    let items: Vec<(String, Vec<u8>)> = (0..SET_MANY_SIZE)
        .map(|i| (format!("batch_key_{}", i), format!("batch_value_{}", i).into_bytes()))
        .collect();
    ...
    let elapsed = start.elapsed();
    println!("  ⏱️  set_many({} items) x10:   {:?}", SET_MANY_SIZE, elapsed);
    ...
    let elapsed = start.elapsed();
    println!("  ⏱️  delete_many({} keys) x100: {:?}", DELETE_MANY_SIZE, elapsed);
    println!("  ⏱️  Average per call:           {:?}", elapsed / 100);
