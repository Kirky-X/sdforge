...
fn main() {
    ...
    println!("  📦 Dataset: {} items across {} prefixes", cache.len(), prefixes.len());
    ...
    for (pattern, description) in &patterns {
        ...
        println!("  ⏱️  Pattern '{}' ({}): {:?} - Found {} matches", 
                 pattern, description, elapsed, matches.len());
    }
    ...
    println!("  ⏱️  Average per call:               {:?}", with_index_time / 10);
    ...
    println!("  ⏱️  Total time for {} prefixes: {:?}", prefixes_to_test.len(), total_time);
    println!("  ⏱️  Average per prefix:           {:?}", total_time / prefixes_to_test.len() as u32);
