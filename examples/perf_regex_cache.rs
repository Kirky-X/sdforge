...
fn main() {
    ...
    println!(
        "⏱️  First call (session:*):   {:?} - Found {} keys",
        session_first,
        keys.len()
    );
    ...
    println!(
        "⏱️  Second call (session:*):  {:?} - Found {} keys",
        session_second,
        keys.len()
    );
    ...
    println!(
        "⏱️  First call (user:1*):     {:?} - Found {} keys",
        complex_first,
        keys.len()
    );
    ...
    println!(
        "⏱️  Second call (user:1*):    {:?} - Found {} keys",
        complex_second,
        keys.len()
    );
    ...
    println!("\n📊 Performance Summary:");
    println!(
        "  user:* pattern:        {:.2}x faster (cached vs first)",
        first_call.as_secs_f64() / second_call.as_secs_f64().max(0.0001)
    );
    println!(
        "  session:* pattern:     {:.2}x faster (cached vs first)",
        session_first.as_secs_f64() / session_second.as_secs_f64().max(0.0001)
    );
    println!(
        "  user:1* pattern:       {:.2}x faster (cached vs first)",
